...
def main() -> None:
    ...
    print("k,H_inf_approx,crossover_repeat")
    for k in ks:
        h_inf = estimate_H_inf_for_k(repeats, h_test, k)
        cross = find_crossover_repeat(repeats, h_test, k)
        h_inf_str = f"{h_inf:.6f}" if h_inf is not None else ""
        cross_str = str(cross) if cross is not None else ""
        print(f"{k},{h_inf_str},{cross_str}")
